...
async function testContactForms() {
  ...
  try {
    ...
    if (contactResponse.status === 200) {
      ...
      testResults.contactForms.issues.push(`Contact page returned status ${contactResponse.status}`);
    ...
    if (leadResponse.status === 200) {
      ...
      testResults.contactForms.failed++;
      testResults.contactForms.issues.push(`Lead API returned status ${leadResponse.status}: ${leadResponse.data}`);
...
async function testPropertySearch() {
  ...
  try {
    ...
    if (propertiesResponse.status === 200) {
      ...
      testResults.propertySearch.issues.push(`Properties page returned status ${propertiesResponse.status}`);
    ...
    if (searchResponse.status === 200) {
      ...
      testResults.propertySearch.failed++;
      testResults.propertySearch.issues.push(`Search page returned status ${searchResponse.status}`);
...
function testEnvironmentConfig() {
  ...
  const requiredEnvVars = [
    'NEXT_PUBLIC_FOLLOW_UP_BOSS_API_KEY'
  ];
