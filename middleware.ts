...
function isLegitimateUser(request: NextRequest): boolean {
  ...
  const hasLegitimatePattern = legitimatePatterns.some(
    (pattern) => pattern.test(userAgent) || pattern.test(referer) || pattern.test(path)
  );

  // Check for bot patterns
  const hasBotPattern = botPatterns.some(
    (pattern) => pattern.test(userAgent) || pattern.test(referer)
  );
